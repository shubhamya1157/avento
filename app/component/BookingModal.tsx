  ...
  if (requested) {
    ...
    return (
      <div className="flex flex-col items-center px-8 py-12 text-center">
        ...
        <h3 className="mt-6 text-2xl font-black tracking-wider text-white">REQUEST SENT</h3>
        <p className="mt-3 max-w-sm text-sm leading-relaxed text-zinc-400">
          Your request for the{" "}
          <span className="font-semibold text-white">
            {vehicle.brand} {vehicle.model}
          </span>{" "}
          has been sent to the owner. Once they accept it, you can pay from My
          Bookings to lock it in.
        </p>

        {/* Request summary — dates, duration, and the amount the SERVER computed
            (this is what you'll pay once the owner accepts). */}
        <div className="mt-7 w-full space-y-3 rounded-2xl border border-white/10 bg-zinc-900/50 p-5 text-left text-sm">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-lg font-black text-white">₹{requested.totalAmount}</span>
          </div>
          <p className="text-[11px] text-zinc-500">Awaiting the owner&apos;s approval — no payment taken yet.</p>
        </div>
